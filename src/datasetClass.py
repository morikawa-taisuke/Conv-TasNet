...
def load_path_csv(dataset_path: str, mix_column: str, target_column: str):
	"""
	csvファイルから指定された列のパスを読み込む (pandas使用)

	Parameters
	----------
	dataset_path: csvのパス
	mix_column: 入力として使用する列名 (例: 'mixture_path')
	target_column: 教師として使用する列名 (例: 'clean_speech_path')
	...
	print(f"  Input (Mix) column: '{mix_column}'")
	...
	# 必要な列が存在するか確認
	if mix_column not in df.columns or target_column not in df.columns:
		print(f"Error: Columns '{mix_column}' or '{target_column}' not found in CSV.")
		print(f"Available columns are: {list(df.columns)}")
		return [], []

	# パスをリストとして抽出
	mix_path_list = df[mix_column].tolist()
	target_path_list = df[target_column].tolist()

	print(f'Found {len(mix_path_list)} files.')
...
class TasNet_dataset_csv(torch.utils.data.Dataset):
	...
	def __init__(self, dataset_path: str, channel: int, device, mix_column: str = 'mix_path', target_column: str = 'target_path'):
		"""
		初期化
		...
		channel(int): チャンネル数
		device: デバイス
		mix_column(str): 入力として使用するCSVの列名
		target_column(str): 教師として使用するCSVの列名
		"""
		""" データの読み込み """
