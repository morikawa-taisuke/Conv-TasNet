...
class TasNet(nn.Module):
	...
	def patting_signal(self, input):
		...
		if rest > 0:
			...
			# pad = torch.zeros(batch_size, 1, rest).to(input)
			pad = zero_tensr.to(input)
			# print(f'pad.size():{pad.size()}')
			input = torch.cat([input, pad], 2)

		pad_aux = torch.zeros(batch_size, self.channel, self.stride).to(input)
...
class type_A(nn.Module):
	...
	def forward(self, input):
		...
		# print(f'input.shape:{input.shape}') #input.shape[バッチサイズ, マイク数, 音声長]
		...
		encoder_output = torch.empty(self.channel, self.encoder_dim, dim_length).to(DEVICE)
		...
		decoder_output = torch.empty(self.channel, wave_length).to(DEVICE)
...
class type_E(nn.Module):
	...
	def patting_signal(self, input):
		...
		pad_aux = torch.zeros(batch_size, self.num_mic, self.stride).to(input)
		# print(f'pad_aux.size():{pad_aux.size()}')
		input = torch.cat([pad_aux, input, pad_aux], 2)
	...
	def forward(self, input):
		...
		# print('\nencoder')
		dim_length = self.get_dim_length(input_patting) - 1
		encoder_output = torch.empty(self.num_mic, self.encoder_dim, dim_length).to(DEVICE)
		for idx, input in enumerate(input_patting[0]):
			# input=input.unsqueeze(0)
			# print_name_type_shape('input:0',input)
			input = input.unsqueeze(0)  # 次元の追加 [128000]->[1,128000]
		...
		decoder_input = masked_output.squeeze()
		# print_name_type_shape('decoder_input',decoder_input)
		decoder_output = torch.empty(self.num_mic, wave_length).to(DEVICE)
		for idx, input in enumerate(decoder_input):
			# print_name_type_shape('input',input)
			output = self.decoder(input)  # B*C, 1, L
			# print_name_type_shape(f'[{idx}]0:decoder_output',output)
...
class type_D_2(nn.Module):
	...
	def patting_signal(self, input):
		...
		pad_aux = torch.zeros(input.size(0), self.num_mic, self.stride).to(input)
		# print(f'pad_aux.size():{pad_aux.size()}')
		input = torch.cat([pad_aux, input, pad_aux], 2)
...
class single_to_maulti_4(nn.Module):
	...
	def patting_signal(self, input):
		...
		pad_aux = torch.zeros(input.size(0), 1, self.stride).to(
			input)  # torch.zero(input.size(0), 入力のマイク数, ストライド)
		# print(f'pad_aux.size():{pad_aux.size()}')
		input = torch.cat([pad_aux, input, pad_aux], 2)
...
class type_D_2_2stage(nn.Module):
	...
	def forward(self, input):
		...
		# print_name_type_shape('encoder_output',encoder_output)
		denoise_output = torch.empty(self.num_mic, wave_length).to(DEVICE)
		estimation_output = torch.empty(self.num_mic, wave_length).to(DEVICE)
...
class single_to_multi(nn.Module):
	...
	def patting_signal(self, input):
		...
		pad_aux = torch.zeros(input.size(0), 1, self.stride).to(input)
		# print(f'pad_aux.size():{pad_aux.size()}')
		# print(f"input.shape:{input.shape}")
