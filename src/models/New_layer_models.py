...
class DepthConv1d_D_2(nn.Module):
	...
	def forward(self, input):
		# input shape: [B, M, BN_dim, L]
		batch_size = input.size(0)
		num_mic = input.size(1)
		bn_dim = input.size(2)
		length = input.size(3)

		""" 1×1-conv """
		# [B*M, BN_dim, L] に変形して 1D 畳み込み
		output = input.view(batch_size * num_mic, bn_dim, length)
		output = self.conv1d(output)  # [B*M, hidden_dim, L]
		output = self.nonlinearity1(output)

		# 1D用の形状 [B*M, hidden_dim, L] のまま正規化を行う
		# これにより reg1 (GroupNorm) は hidden_dim をチャンネルとして正しく認識します
		output = self.reg1(output)

		""" ch数の削減 M ch → 1 ch """
		# Conv2d 用に [B, M, hidden_dim, L] に変形
		# nn.Conv2d(in_channels=num_mic, ...) なので M がチャンネル次元にくる必要がある
		output = output.view(batch_size, num_mic, -1, length)
		output = self.conv2d(output)  # [B, 1, hidden_dim, L]

		output = self.nonlinearity1(output)

		# ここでの正規化がエラーの箇所です。
		# [B, 1, hidden_dim, L] のままだと GroupNorm は「1」をチャンネルと見なしてしまいます。
		# 一旦 [B, hidden_dim, L] に戻してから正規化します。
		output = output.squeeze(1)  # [B, hidden_dim, L]
		output = self.reg1(output)  # hidden_dim をチャンネルとして正規化

		""" D-conv (Depthwise 1D) """
		# そのまま 1D 畳み込みを実行
		output = self.dconv1d(output)  # [B, hidden_dim, L]

		if self.causal:
			output = self.nonlinearity2(output[:, :, :-self.padding])
		else:
			output = self.nonlinearity2(output)
		output = self.reg2(output)  # reg2 も [B, hidden_dim, L] で受ける

		""" ch数の拡張 1ch → M ch """
		# [B, 1, hidden_dim, L] にしてから拡張
		output = output.unsqueeze(1)
		output = self.inversion_Conv2d(output)  # [B, M, hidden_dim, L]

		output = self.nonlinearity1(output)

		# 拡張後も同様に [B*M, hidden_dim, L] に戻して正規化
		output = output.view(batch_size * num_mic, -1, length)
		output = self.reg1(output)

		""" 残差接続 """
		# res_out (1x1 Conv1d) を適用
		residual = self.res_out(output)  # [B*M, BN_dim, L]
		residual = residual.view(batch_size, num_mic, -1, length)  # 元の形に戻す

		""" スキップ接続 """
		if self.skip:
			skip = self.skip_out(output)  # [B*M, BN_dim, L]
			skip = skip.view(batch_size, num_mic, -1, length)
			return residual, skip
		else:
			return residual
...
class TCN_D_2(nn.Module):
	...
	def __init__(self, input_dim, output_dim, BN_dim, hidden_dim, layer, stack, kernel=3, skip=True, causal=False, dilated=True,
		...
		for s in range(stack):  # 1-DConvブロックの塊をstack回繰り返す
			for i in range(layer):  # layer個の1-DConvブロック
				...
				if self.dilated:  # (True)
					self.TCN.append(
						DepthConv1d_D_2(BN_dim, hidden_dim, kernel, dilation=2 ** i, padding=2 ** i, skip=skip, causal=causal,
						                num_mic=num_mic))
				else:
					self.TCN.append(DepthConv1d(BN_dim, hidden_dim, kernel, dilation=1, padding=1, skip=skip, causal=causal))
				""" 受容野の更新 """
	...
	def forward(self, input):
		# input shape: [B, M, input_dim, L]
		batch_size = input.size(0)
		num_mic = input.size(1)
		length = input.size(3)

		""" normalization """
		# GroupNormなどは [B, C, L] 形式を期待するため、一旦 B*M をバッチとする
		output = input.view(batch_size * num_mic, -1, length)
		output = self.LN(output)

		""" Bottleneck Layer """
		output = self.BN1d(output)  # [B*M, BN_dim, L]

		# TCNブロック（DepthConv1d_D_2）に渡すために [B, M, BN_dim, L] に戻す
		output = output.view(batch_size, num_mic, -1, length)
		...
		if self.skip:
			...
			for i in range(len(self.TCN)):
				residual, skip = self.TCN[i](output)
				output = output + residual  # 残差接続
				skip_connection = skip_connection + skip  # スキップ接続
		else:
			for i in range(len(self.TCN)):
				output = output + self.TCN[i](output)

		""" output layer """
		# 最終出力用に [B*M, BN_dim, L] に変形
		if self.skip:
			output_flat = skip_connection.view(batch_size * num_mic, -1, length)
		else:
			output_flat = output.view(batch_size * num_mic, -1, length)

		output = self.output(output_flat)  # [B*M, output_dim, L]

		# [B, M, output_dim, L] に戻して完了
		return output.view(batch_size, num_mic, -1, length)