...
if __name__ == "__main__":
    ...
    for wave_type in wave_type_list:
        for model_type in model_list:
            dataset_dir = f"{const.DATASET_DIR}/{base_name}/all_angle"
            pth_dir = f"{const.PTH_DIR}/{base_name}/all_angle"
            main(dataset_path=os.path.join(dataset_dir, wave_type),
                 out_path=os.path.join(pth_dir, f"{base_name}_{wave_type}_{model_type}_all_angle"),
                 train_count=200,
        ...
        for wave_type in wave_type_list:
            for angle in angle_list:
                ...
                test.test(mix_dir=os.path.join(mix_dir, wave_type),
                          ...
                          model_name=os.path.join(pth_dir, f"{base_name}_{wave_type}_{model_type}", f"BEST_{base_name}_{wave_type}_{model_type}.pth"),
                          channel=channel,
                          model_type=model_type)
