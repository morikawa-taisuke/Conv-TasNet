...
for angle in angle_list:
    dir_name = f'subset_DEMAND_hoth_1010dB_05sec_{ch}ch_circular_10cm_Dtype'
    out_dir = f'{const.OUTPUT_WAV_DIR}/{dir_name}/{angle}'
    wave_path = f"{const.MIX_DATA_DIR}\\subset_DEMAND_hoth_1010dB_05sec_{ch}ch_circular_10cm\\{angle}\\test\\"
    ...
    for wave_type in wave_type_list:
        ...
        print('evaluation')
        eval.main(target_dir=f"{const.MIX_DATA_DIR}\\subset_DEMAND_hoth_1010dB_05sec_{ch}ch_circular_10cm\\{angle}\\test\\clean",
                  estimation_dir=f'{out_dir}/{wave_type}',
                  out_path=f'{const.EVALUATION_DIR}\\subset_DEMAND_hoth_1010dB_05sec_{ch}ch_circular_10cm\\{angle}\\{wave_type}.csv',
                  condition=condition,
                  channel=ch)
