...
def main():
    ...
    train_dataset_path = path_config["train_dataset"]
    valid_dataset_path = path_config.get("validation_dataset") # 検証データパス（オプション）
    ...
    target_col = dataset_params["target_column"]
    max_length_sec = dataset_params.get("max_length_sec")
    ...
    train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, pin_memory=True, collate_fn=CsvDataset.collate_fn if model_config["type"] == "enhance" else None)
    ...
    if valid_dataset_path:
        if model_config["type"] == "enhance":
            ...
            valid_dataset = TasNet_dataset_csv_separate(valid_dataset_path, channel=1, device=device, mix_column=mix_col, target_column=target_col)
        else: # multi-channel separate
            valid_dataset = TasNet_dataset(valid_dataset_path)
        
        valid_loader = DataLoader(dataset=valid_dataset, batch_size=batch_size, shuffle=False, pin_memory=True, collate_fn=CsvDataset.collate_fn if model_config["type"] == "enhance" else None)
